...
understand how it works, you're at the right place!
...
> by this user. There is no permissions or other features for multi-user
> collaboration, and the entire security model (see below) asummes so.
...
The daemon syncs changes as soon as they are snapshotted by `jj`, and we don't
propagate deletions such as `jj op abandon` (see *Consistency* below). We also
perform automatic snapshots by default. Per `jj`'s design, content of
snapshots may live indefinitely as long as it is referenced by the op log.
This means **secrets can be replicated across every machine of the mesh** (and
future ones, if still referencable by the op log) if they are ever
snapshotted.
...
As `jj-mesh` synchronizes the full history of repositories, we make the best
efforts to ensure no data ever gets corrupted or lost in the process, and that
users can always restore to a good state. This is thankfully made easy by
`jj`'s design.
...
- **Updates are atomic.** Especially when touching git storage on collocated
...
- **`jj` version is strictly enforced.** As synchronization depends on
  internals of jj's default backend, we enforce that users have a compatible
  jj version and that no custom backend is in use before performing any sync.
